...
namespace Lesson3
{
    class Utilities
    {
        ...
        public static Complex PrintAndReadAsComplex(string msg, bool newline = false, bool newlineAfterMsg = true)
        {
            // не стал пока разбираться с дженериками, т. к. они наверняка будут позже
            Print(msg, newlineAfterMsg: newlineAfterMsg);
            double realPart = PrintAndReadAsDouble("Введите вещественную часть числа", newlineAfterMsg: false);
            double imaginaryPart = PrintAndReadAsDouble("Введите мнимую часть числа", newlineAfterMsg: false);
            if (newline)
            {
                Print();
            }
